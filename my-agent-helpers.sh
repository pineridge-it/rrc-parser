...
am_read_message() {
    ...
    if [ -z "$message_id" ]; then
        echo "Usage: am_read_message <message_id>"
        return 1
    fi
    curl -s -X POST -H "Content-Type: application/json" -d "{\"jsonrpc\": \"2.0\", \"id\": 1, \"method\": \"resources/read\", \"params\": {\"uri\": \"resource://message/$message_id\"}}" "$AGENT_MAIL_URL" | jq -r '.result.contents[0].text'
}