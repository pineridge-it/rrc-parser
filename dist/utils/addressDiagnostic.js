...
const daaddressSchema = config.schemas['11'];
// FIXED: Add null check + throw helpful error if field missing
const addressLineField = daaddressSchema.fields.find((f) => f.name === 'address_line');
if (!addressLineField) {
...
const daaddressLines = [];
lines.forEach((line, idx) => {
    if (line.startsWith('11'))
        daaddressLines.push({ lineNum: idx + 1, text: line });
});
...
daaddressLines.slice(0, 10).forEach(rec => {
    ...
    if (dataAfter)
        console.log(`  Data After Pos ${realEnd}: "${dataAfter}"`);
    console.log(`  Raw: ${raw}`);
});
...
//# sourceMappingURL=addressDiagnostic.js.map